        ...
        try {
            ...
            client.newCall(request).execute().use { response ->
                if (response.isSuccessful) {
                    ...
                    if (code == 200) {
                        val dataObj = parsed["data"]?.jsonObject ?: return@withContext false
                        ...
                        var finalSsoToken = dataObj["ssoToken"]?.jsonPrimitive?.content ?: ""
                        var finalCrm = dataObj["sessionAttributes"]?.jsonObject?.get("user")?.jsonObject?.get("subscriberId")?.jsonPrimitive?.content ?: ""
                        ...
                        if (tempToken.isNotBlank() && initialAuth.isBlank()) {
                            ...
                            val expireReq = Request.Builder()
                                .url("$BASE_URL/device/logoutall") // Replicating expireallusers protocol
                            ...
                            client.newCall(expireReq).execute().use { expireRes ->
                                ...
                                try {
                                    ...
                                    if (expData != null && expData["ssoToken"] != null) {
                                        finalSsoToken = expData["ssoToken"]?.jsonPrimitive?.content ?: finalSsoToken
                                    }
                                } catch (e: Exception) {}
                            }
                        }
                        
                        if (finalSsoToken.isNotBlank()) {
                            cachedToken = finalSsoToken
                            cachedCrm = finalCrm
